...
class MistralChat(Model):
    ...
    def _get_client_params(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            get_logger().error("MISTRAL_API_KEY not set. Please set the MISTRAL_API_KEY environment variable.")
    ...
    def invoke(self, messages: List[Message]) -> Union[ChatCompletionResponse, ParsedChatCompletionResponse]:
        ...
        try:
            ...
            get_logger().error(f"HTTPValidationError from Mistral: {e}")
            raise ModelProviderError(message=str(e), model_name=self.name, model_id=self.id) from e
        except SDKError as e:
            get_logger().error(f"SDKError from Mistral: {e}")
            raise ModelProviderError(message=str(e), model_name=self.name, model_id=self.id) from e
