...
def main():
    ...
    # App header
    ####################################################################
    st.markdown("<h1 class='main-title'>Agentic RAG v2.0</h1>", unsafe_allow_html=True)
    st.markdown(
        "<p class='subtitle'>Your intelligent research assistant powered by Agno v2 primitives</p>",
    ...
    if (
        ...
        logger.info("---*--- Creating new Agentic RAG v2.0  ---*---")
        agentic_rag_agent = get_agentic_rag_agent(model_id=model_id, user_id='streamlit_user')
    ...
    # Display document count and status
    try:
        from agentic_rag import get_document_stats
        doc_stats = get_document_stats()
        
        # Create a status display
        total_docs = doc_stats["vector_store_count"]
        if total_docs > 0:
            st.sidebar.success(f"📊 {total_docs} documents loaded")
        else:
            st.sidebar.info("📊 No documents loaded")
        
        # Show errors if any
        if doc_stats["vector_store_error"]:
            st.sidebar.error(f"VectorStore Error: {doc_stats['vector_store_error']}")
            
        # Show source breakdown if documents exist
        if doc_stats["document_sources"]:
            source_counts = {}
            for source in doc_stats["document_sources"]:
                source_counts[source] = source_counts.get(source, 0) + 1
            
    except Exception as e:
        st.sidebar.error(f"Error getting document stats: {str(e)}")
    ...
    if (
        ...
        if input_url not in st.session_state.loaded_urls:
            ...
            if docs:
                # Use v2 Knowledge API - add each document as a source
                for i, doc in enumerate(docs):
                    agentic_rag_agent.knowledge.add_source(
                        name=f"URL_{i}",
    ...
    if st.sidebar.button("Clear Knowledge Base"):
        # Use v2 knowledge clearing method
            ...
            with st.spinner("🤔 Thinking..."):
                ...
                try:
                    ...
                    for _resp_chunk in run_response:
                        ...
                        # Handle ModelResponse objects with tool_executions
                        if hasattr(_resp_chunk, "tool_executions") and _resp_chunk.tool_executions:
                            display_tool_calls(tool_calls_container, _resp_chunk.tool_executions)
                        # Handle ToolCallStartedEvent and ToolCallCompletedEvent with tool attribute
                        elif hasattr(_resp_chunk, "tool") and _resp_chunk.tool:
                            display_tool_calls(tool_calls_container, [_resp_chunk.tool])

                        # Display response
                        if _resp_chunk.content is not None:
                            # Filter out timing messages that appear below tool calls
                            content = _resp_chunk.content
                            if not ("completed in" in content and content.strip().endswith("s.")):
                                response += content
