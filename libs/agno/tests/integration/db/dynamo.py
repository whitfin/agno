...
# TODO: should use localstack or actual AWS DynamoDB for testing
...
def test_db(dynamo_client) -> Generator[DynamoDb, None, None]:
    ...
    table_prefix = f"test_agno_{uuid.uuid4().hex[:8]}"
    db = DynamoDb(
        db_client=dynamo_client,
        region_name=TEST_REGION,
        session_table=f"{table_prefix}_sessions",
        memory_table=f"{table_prefix}_memories",
        metrics_table=f"{table_prefix}_metrics",
        eval_table=f"{table_prefix}_evals",
        knowledge_table=f"{table_prefix}_knowledge",
    )

    # Create tables
    db._create_table_if_not_exists("sessions")
    db._create_table_if_not_exists("memories")
    db._create_table_if_not_exists("metrics")
    db._create_table_if_not_exists("evals")
    db._create_table_if_not_exists("knowledge")
...
class TestDynamoDbInfrastructure:
    ...
    def test_initialization_with_region(self):
        ...
        db = DynamoDb(region_name=TEST_REGION)

        assert db.region_name == TEST_REGION
        assert db.db_client is not None
        assert db.session_table_name == "agno_sessions"

    def test_initialization_with_client(self, dynamo_client):
        db = DynamoDb(db_client=dynamo_client)

        assert db.db_client == dynamo_client
        assert db.region_name is None
        assert db.session_table_name == "agno_sessions"
    ...
    def test_initialization_requires_client_or_region(self):
    ...
    def test_create_table_if_not_exists(self, test_db: DynamoDb):
        ...
        # Just verify they exist by checking table status
        table_name = test_db.session_table_name
        response = test_db.db_client.describe_table(TableName=table_name)
...
class TestDynamoDbSession:
    ...
    def test_insert_session(self, test_db: DynamoDb, sample_agent_session: AgentSession):
        ...
        response = test_db.db_client.get_item(
            TableName=test_db.session_table_name,
            Key={'id': {'S': 'test_agent_session_1'}}
        )
    ...
    def test_get_sessions_with_user_filter(self, test_db: DynamoDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_with_session_type_filter(self, test_db: DynamoDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_no_filters(self, test_db: DynamoDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_with_limit(self, test_db: DynamoDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
