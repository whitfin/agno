    ...
    with Live(console=console) as live_console:
        ...
        if markdown:
            for member in team.members:
                ...
                if isinstance(member, Team) and member.id is not None:
                    member_markdown[member.id] = True
            team_markdown = True
        ...
        for member in team.members:
            ...
            if member.response_model is not None and isinstance(member, Team) and member.id is not None:
                member_markdown[member.id] = False  # type: ignore
                ...
                for member_response in run_response.member_responses:
                    ...
                    if hasattr(member_response, "tools") and member_response.tools:
                        ...
                        if isinstance(member_response, RunOutput) and member_response.id is not None:
                            member_name = team._get_member_name(member_response.id)
