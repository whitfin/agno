...
from agno.run_v2.messages import RunMessages
from agno.run_v2.response import RunResponse, RunResponseDelta, RunEvent, RunResponseExtraData
from agno.storage.base import Storage
from agno.session_v2.agent import AgentSession
...
class Agent:
    ...
    # --- User settings ---
    # User ID (used as the default user_id for memory if not specified during runs)
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        retries = retries or self.retries or 3
        if retries < 1:
        ...
        if self.memory and not user_id:
            log_debug("No user_id provided, creating default user_id")
            user_id = str(uuid4())

        # Read existing session from storage
        session = self._read_session_from_storage(session_id=session_id)
        ...
        # Set run_input
        run_response = RunResponse(run_id=run_id, 
        ...
        for attempt in range(num_attempts):
            ...
            log_debug(f"Agent Run Start: {self.run_id}", center=True)

            run_messages: RunMessages = self.get_run_messages(
                run_response=run_response,
                message=message, 
                audio=audio, 
                images=images, videos=videos, files=files, messages=messages, **kwargs
            )
            ...
            try:
                if stream:
                    ...
                    # Add the run_response to the session
                    session.add_run(run_response)
    ...
    def _run(
        ...
        12. Save output to AgentMemorye_response_to_file is set
        """
        ...
        if model_response.tool_calls is not None:
            if run_response.tool_calls is None:
                run_response.tool_calls = model_response.tool_calls
            else:
                run_response.tool_calls.extend(model_response.tool_calls)
        ...
        if run_messages.system_message is not None:
            # TODO: Add system message to session history
            pass

        # TODO: Add other messages to session history
        ...
        # TODO: Get rid of AgentRun, we only need RunResponse stored on a session
        ...
        if (
            ...
            # TODO: Don't use just user message, use extra messages as well
            self.memory.update_memory(input=run_messages.user_message.get_content_string())

        # Update the session summary if needed
        if self.memory.create_session_summaries and self.memory.update_session_summary_after_run and user_id:
            self.memory.create_session_summary(user_id=user_id, session=session)

        # 10. Calculate session metrics
        # TODO: Update metrics on a session from all the messages on the session
        # self.session_metrics = self.calculate_session_metrics(self.memory.messages)

        # 11. Save session to storage
        self._write_session_to_storage(session=session)

        # 12. Save output to file if save_response_to_file is set
        # TODO: Fix
        # self.save_run_response_to_file(message=run_messages.user_message)
    ...
    def create_run_response(
        ...
        *,
        ...
        rr = RunResponse(
            ...
            tools=self.run_response.tool_calls,
    ...
    def _read_session_from_storage(self, session_id: str) -> Optional[AgentSession]:
        ...
        """
        if self.storage is not None:
            agent_session = cast(AgentSession, self.storage.read(session_id=session_id))

            if agent_session is not None:
                self.load_agent_session(session=agent_session)
            self.load_user_memories()
        return self.agent_session

    def _write_session_to_storage(self, session: AgentSession) -> Optional[AgentSession]:
        ...
        """
        if self.storage is not None:
            agent_session = cast(AgentSession, self.storage.upsert(session=session))
        ...
        **kwargs: Any,
        ...
        if self.add_references and message:
            ...
            if docs_from_knowledge is not None:
                ...
                if run_response.extra_data.references is None:
                    run_response.extra_data.references = []
                run_response.extra_data.references.append(references)
        ...
        **kwargs: Any,
        ...
        if self.add_messages is not None:
            ...
            if len(messages_to_add_to_run_response) > 0:
                ...
                if run_response.extra_data is None:
                    run_response.extra_data = RunResponseExtraData(add_messages=messages_to_add_to_run_response)
                else:
                    if run_response.extra_data.add_messages is None:
                        run_response.extra_data.add_messages = messages_to_add_to_run_response
                    else:
                        run_response.extra_data.add_messages.extend(messages_to_add_to_run_response)

        # 3. Add history to run_messages
        if self.add_history_to_messages:
            from copy import deepcopy

            # TODO: Get history from session
