import logging
from typing import Optional

from fastapi.routing import APIRouter

from agno.agent.agent import Agent
from agno.app.interfaces.slack.async_router import attach_async_routes
from agno.app.interfaces.slack.sync_router import attach_sync_routes
from agno.team.team import Team

logger = logging.getLogger(__name__)


class SlackAPI:
    type = "slack"

    router: APIRouter

    def __init__(self, agent: Optional[Agent] = None, team: Optional[Team] = None):
        self.agent = agent
        self.team = team
        
        if not self.agent and not self.team:
            raise ValueError("SlackAPI requires an agent and a team")
        
    def get_router(self, use_async: bool = True) -> APIRouter:
        # Cannot be overridden
        prefix: str = "/slack"
        version: str = "/v1"
        self.router = APIRouter(prefix=prefix + version, tags=["Slack"])

        if use_async:
            self.router = attach_async_routes(router=self.router, agent=self.agent, team=self.team)
        else:
            self.router = attach_sync_routes(router=self.router, agent=self.agent, team=self.team)
            
        return self.router
