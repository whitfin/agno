...
class Groq(Model):
    ...
    def _get_client_params(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            ...
            if not self.api_key:
                get_logger().error("GROQ_API_KEY not set. Please set the GROQ_API_KEY environment variable.")
    ...
    def format_message(self, message: Message) -> Dict[str, Any]:
        ...
        if message.audio is not None:
            get_logger().warning("Audio input is currently unsupported.")
    ...
    def invoke(self, messages: List[Message]) -> ChatCompletion:
        ...
        try:
            ...
            get_logger().error(f"Error calling Groq API: {str(e)}")
            ...
            get_logger().error(f"Unexpected error calling Groq API: {str(e)}")
    ...
    def parse_provider_response(self, response: ChatCompletion) -> ModelResponse:
        ...
        if response_message.tool_calls is not None and len(response_message.tool_calls) > 0:
            try:
                ...
                get_logger().warning(f"Error processing tool calls: {e}")
