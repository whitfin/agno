"""Integration tests for the SingleStoreDb class"""
...
# TODO: should spin up a test SingleStore container
TEST_DB_URL = "mysql+pymysql://root:password@localhost:3307/test_agno"


@pytest.fixture(scope="session")
def engine():
    """Handle the SingleStore engine lifecycle"""
    try:
...
def test_db(engine) -> Generator[SingleStoreDb, None, None]:
    """SingleStoreDb instance to be used across all tests"""
...
class TestSingleStoreDbInfrastructure:
    """Tests for the infrastructure-related methods of SingleStoreDb"""
    ...
    def test_initialization_with_db_url(self):
        ...
        db = SingleStoreDb(db_url=TEST_DB_URL)
    ...
    def test_initialization_with_engine(self, engine):
        db = SingleStoreDb(db_engine=engine)
    ...
    def test_initialization_with_custom_schema(self, engine):
        ...
        db = SingleStoreDb(db_engine=engine, db_schema=custom_schema)
    ...
    def test_create_table(self, test_db: SingleStoreDb):
    ...
    def test_get_table(self, test_db: SingleStoreDb):
    ...
    def test_get_table_all_mappings(self, test_db: SingleStoreDb):
    ...
    def test_get_table_invalid_type(self, test_db: SingleStoreDb):
...
class TestSingleStoreDbSession:
    """Tests for session-related operations in SingleStoreDb"""

    @pytest.fixture(autouse=True)
    def cleanup_sessions(self, test_db: SingleStoreDb):
        yield
        table = test_db._get_table("sessions")
        with test_db.Session() as session:
    ...
    def test_insert_session(self, test_db: SingleStoreDb, sample_agent_session: AgentSession):
    ...
    def test_upsert_session_agent(self, test_db: SingleStoreDb, sample_agent_session: AgentSession):
    ...
    def test_upsert_session_team(self, test_db: SingleStoreDb, sample_team_session: TeamSession):
    ...
    def test_update_session(self, test_db: SingleStoreDb, sample_agent_session: AgentSession):
    ...
    def test_get_session_by_id_not_found(self, test_db: SingleStoreDb):
        """Test retrieving non-existent session"""
        result = test_db.get_session_by_id("non_existent_session")
        assert result is None

    def test_get_session_without_deserialization(self, test_db: SingleStoreDb, sample_agent_session: AgentSession):
    ...
    def test_delete_session(self, test_db: SingleStoreDb, sample_agent_session: AgentSession):
    ...
    def test_get_sessions_with_user_filter(self, test_db: SingleStoreDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_with_session_type_filter(self, test_db: SingleStoreDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_no_filters(self, test_db: SingleStoreDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
    ...
    def test_get_sessions_with_limit(self, test_db: SingleStoreDb, sample_agent_session: AgentSession, sample_team_session: TeamSession):
...
class TestSingleStoreDbMemory:
    """Tests for memory-related operations in SingleStoreDb"""

    @pytest.fixture(autouse=True)
    def cleanup_memories(self, test_db: SingleStoreDb):
        yield
        table = test_db._get_table("memories")
        with test_db.Session() as session:
            session.execute(table.delete())
    ...
    def test_upsert_memory(self, test_db: SingleStoreDb, sample_memory: UserMemory):
    ...
    def test_get_memory_by_id_not_found(self, test_db: SingleStoreDb):
        """Test retrieving non-existent memory"""
        result = test_db.get_memory_by_id("non_existent_memory")
        assert result is None

    def test_delete_memory(self, test_db: SingleStoreDb, sample_memory: UserMemory):
    ...
    def test_get_memories_with_user_filter(self, test_db: SingleStoreDb):
...
class TestSingleStoreDbMetrics:
    """Tests for metrics-related operations in SingleStoreDb"""

    @pytest.fixture(autouse=True)
    def cleanup_metrics(self, test_db: SingleStoreDb):
        yield
        table = test_db._get_table("metrics")
        with test_db.Session() as session:
            session.execute(table.delete())
    ...
    def test_calculate_metrics_basic(self, test_db: SingleStoreDb):
...
class TestSingleStoreDbEvals:
    """Tests for evaluation-related operations in SingleStoreDb"""

    @pytest.fixture(autouse=True)
    def cleanup_evals(self, test_db: SingleStoreDb):
        yield
        table = test_db._get_table("evals")
        with test_db.Session() as session:
            session.execute(table.delete())
    ...
    def test_upsert_eval_record(self, test_db: SingleStoreDb, sample_eval_record: EvalRunRecord):
    ...
    def test_get_eval_record_by_id_not_found(self, test_db: SingleStoreDb):
        """Test retrieving non-existent evaluation record"""
        result = test_db.get_eval_record_by_id("non_existent_eval")
        assert result is None

    def test_delete_eval_record(self, test_db: SingleStoreDb, sample_eval_record: EvalRunRecord):
    ...
    def test_get_eval_records_with_filters(self, test_db: SingleStoreDb, sample_eval_record: EvalRunRecord):
...
class TestSingleStoreDbKnowledge:
    """Tests for knowledge-related operations in SingleStoreDb"""

    @pytest.fixture(autouse=True)
    def cleanup_knowledge(self, test_db: SingleStoreDb):
        yield
        table = test_db._get_table("knowledge")
        with test_db.Session() as session:
            session.execute(table.delete())
    ...
    def test_upsert_knowledge_row(self, test_db: SingleStoreDb, sample_knowledge_row: KnowledgeRow):
    ...
    def test_get_knowledge_row_by_id_not_found(self, test_db: SingleStoreDb):
        """Test retrieving non-existent knowledge row"""
        result = test_db.get_knowledge_row_by_id("non_existent_knowledge")
        assert result is None

    def test_delete_knowledge_row(self, test_db: SingleStoreDb, sample_knowledge_row: KnowledgeRow):
    ...
    def test_get_knowledge_rows_with_filters(self, test_db: SingleStoreDb, sample_knowledge_row: KnowledgeRow):
