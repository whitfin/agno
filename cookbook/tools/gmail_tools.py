...
# tool = GmailTools(port=8080)  # Use a different port to avoid conflicts
# response: EmailSearchResponse = tool.get_emails_from_user(email, 1)

# print(f"Success: {response.success}")
# print(f"Total emails found: {response.total_count}")
# print(f"Response type: {type(response)}")

# if response.success and response.emails:
#     first_email = response.emails[0]
#     print(f"Message ID: {first_email.id}")
#     print(f"Thread ID: {first_email.thread_id}")
...
#     if mark_response.success:
#         print(f"Labels removed: {mark_response.labels_removed}")
#     else:
#         print(f"Mark as read error: {mark_response.error}")
# elif not response.success:
#     print(f"Error: {response.error}")
# else:
#     print("No emails found from this sender")

# # response: FindEmailOutput = agent.print_response(
