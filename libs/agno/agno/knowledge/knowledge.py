        ...
        for path in path:
            ...
            if path.is_file():
                ...
                for file in path.iterdir():
                    ...
                    self._add_document_by_path(file_document)
            else:
                raise ValueError(f"Invalid path: {path}")

    def _add_document_by_content(self, id: str, document: DocumentV2):
        print("Adding document by content")

        # Add to documents db
        print("Adding to documents db")
        ...
        if document.content:
            if document.content.type == "pdf":
                ...
                # read the document - returns List[Document]
                ...
                # For non-PDF content, handle as before
                self.vector_store.upsert(documents=[document], filters=document.metadata)
                self._add_to_documents_db(document)
                if self.document_store:
