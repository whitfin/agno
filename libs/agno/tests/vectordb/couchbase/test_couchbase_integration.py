...
@pytest.fixture
def couchbase_db(cluster_options: ClusterOptions, search_index: SearchIndex, embedder: OpenAIEmbedder) -> Generator[CouchbaseFTS, None, None]:
...
def test_cluster_level_index(cluster_options: ClusterOptions, search_index: SearchIndex, embedder: OpenAIEmbedder):
    ...
    try:
        ...
        doc = Document(
            name="cluster_test",
            content="Testing cluster level index",
            meta_data={"level": "cluster"}
        )
        db.insert([doc])
