...
@pytest_asyncio.async_def
async def test_get_eval_runs_with_filters(async_postgres_db_real: AsyncPostgresDb, sample_eval_runs: List[EvalRunRecord]):
...
@pytest_asyncio.async_def
async def test_get_eval_runs_with_pagination(async_postgres_db_real: AsyncPostgresDb, sample_eval_runs: List[EvalRunRecord]):
    ...
    runs, total_count = await async_postgres_db_real.get_eval_runs(
        limit=2,
        page=1,
        deserialize=False
    )
...
@pytest_asyncio.async_def
async def test_get_eval_runs_with_sorting(async_postgres_db_real: AsyncPostgresDb, sample_eval_runs: List[EvalRunRecord]):
...
async def test_rename_eval_run(async_postgres_db_real: AsyncPostgresDb, sample_eval_run: EvalRunRecord):
    ...
    result = await async_postgres_db_real.rename_eval_run(
        eval_run_id="test_eval_run_1",
        name="Renamed Evaluation Run"
    )
...
@pytest_asyncio.async_def
async def test_rename_eval_run_deserialize_false(async_postgres_db_real: AsyncPostgresDb, sample_eval_run: EvalRunRecord):
...
async def test_eval_runs_with_multiple_eval_types(async_postgres_db_real: AsyncPostgresDb):
    ...
    filtered_runs = await async_postgres_db_real.get_eval_runs(
        eval_type=[EvalType.agent, EvalType.team]
    )
