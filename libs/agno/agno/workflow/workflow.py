...
from typing import Any, AsyncIterator, Awaitable, Callable, Dict, Iterator, List, Literal, Optional, Union, cast, overload
...
from agno.workflow.types import StepInput, StepMetrics, StepOutput, WorkflowExecutionInput, WorkflowMetrics
...
class Workflow:
    ...
    def _execute(
        ...
        if isinstance(self.steps, Callable):
            ...
            try:
                ...
                self.save_session(session_id=self.session_id, user_id=self.user_id)
    ...
    def get_workflow_session(self, session_id: str, user_id: Optional[str] = None) -> Optional[WorkflowSession]:
        ...
        if self.workflow_session is not None and self.workflow_session.session_id == session_id:
            return self.workflow_session
        
        if self.db is not None:
            self.workflow_session = cast(
                WorkflowSession, self.db.read_session(session_id=session_id, session_type=SessionType.WORKFLOW)
            )
        ...
        self.workflow_session = WorkflowSession(
            ...
            session_data=self.get_workflow_session_data(),
            created_at=int(time()),
        )
        return self.workflow_session

    def save_session(self, session_id: str, user_id: Optional[str] = None) -> Optional[WorkflowSession]:
        """Save the WorkflowSession to storage"""
        if self.db is not None:
            session_to_save = self.get_workflow_session(session_id=session_id, user_id=user_id)
            # Update the session_data with the latest data
            session_to_save.session_data = self.get_workflow_session_data()
            
            self.db.upsert_session(session=session_to_save)
        return self.workflow_session
    ...
    def get_workflow_session_data(self) -> Dict[str, Any]:
