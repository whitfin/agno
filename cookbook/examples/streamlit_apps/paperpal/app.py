...
def restart_session(model_id: str = None):
    ...
    keys_to_clear = [
        "research_topic", "search_terms", "arxiv_results", "exa_results", 
        "final_blog", "research_agents", "messages", "session_id"
    ]
...
def main():
    ...
    # App header
    ####################################################################
    st.markdown("<h1 class='main-title'>Paperpal Research Assistant</h1>", unsafe_allow_html=True)
    st.markdown(
    ...
    ####################################################################
    if "research_agents" not in st.session_state or not st.session_state["research_agents"]:
        st.session_state["research_agents"] = get_paperpal_agents(model_id=selected_model)
    ...
    if prompt := st.chat_input("💭 Ask me anything about research or start a new research project!"):
    ...
    with col1:
        enable_arxiv = st.sidebar.checkbox("📚 ArXiv Search", value=True, help="Search academic papers")
    with col2:
        enable_exa = st.sidebar.checkbox("🌐 Web Search", value=True, help="Search web content")
    ...
    if last_message and last_message.get("role") == "user":
        ...
        if question.startswith("🔬 Research Request:") and st.session_state.get("research_topic"):
...
def process_research_workflow():
    ...
    with st.chat_message("assistant"):
        ...
        if not st.session_state.get("search_terms"):
            with st.status("🔍 Generating strategic search terms...", expanded=True) as status:
                try:
                    search_input = {
                        "topic": topic,
                    ...
                    response = agents["search_term_generator"].run(json.dumps(search_input))
                    if isinstance(response.content, SearchTerms):
                        ...
                        raise ValueError("Invalid response format from search term generator")
                        
                except Exception as e:
                    st.error(f"Error generating search terms: {str(e)}")
                    status.update(label="❌ Search term generation failed", state="error")
                    return
        ...
        # Step 2: ArXiv Search
        if st.session_state.get("enable_arxiv", True) and not st.session_state.get("arxiv_results"):
            with st.status("📚 Searching ArXiv for research papers...", expanded=True) as status:
                try:
                    arxiv_response = agents["arxiv_search_agent"].run(search_terms.model_dump_json(indent=2))
                    if isinstance(arxiv_response.content, ArxivSearchResults):
                        ...
                        if arxiv_response.content.results:
                            ...
                            status.update(label="✅ ArXiv search completed", state="complete", expanded=False)
        ...
        # Step 3: Web Search  
        if st.session_state.get("enable_exa", True) and not st.session_state.get("exa_results"):
            with st.status("🌐 Searching web for current insights...", expanded=True) as status:
                try:
                    exa_response = agents["exa_search_agent"].run(search_terms.model_dump_json(indent=2))
                    if isinstance(exa_response.content, WebSearchResults):
                        ...
                        if exa_response.content.results:
                            ...
                            status.update(label="✅ Web search completed", state="complete", expanded=False)
        ...
        if (arxiv_results or exa_results) and not st.session_state.get("final_blog"):
            with st.status("📝 Generating comprehensive research blog...", expanded=True) as status:
                try:
                    ...
                    research_content += f"## Search Terms\n{search_terms.model_dump_json(indent=2)}\n\n"
                    
                    if arxiv_results:
                        research_content += "## ArXiv Research Papers\n\n"
                        research_content += f"{arxiv_results.model_dump_json(indent=2)}\n\n"
                    
                    if exa_results:
                        research_content += "## Web Research Content\n\n"
                        research_content += f"{exa_results.model_dump_json(indent=2)}\n\n"
